...
def similar(arr1,arr2):
    ...
    sim = numer/demon
    return (sim+1)/2
class TreeHot_F(BertPreTrainedModel):

    def __init__(self, config):
        super(TreeHot_F, self).__init__(config)
    ...
    def forward(
        ...
        for tree in trees:
            for node in tree.nearroot_node:
                if node not in tree.leaf_node:
                    try:
            ...
            for k,nodepaths in tree.node2path_F.items():
                for nodepath in nodepaths:
                    ...
                    for path in nodepath[0][1:]:
                        hidden_ = self.Decoder_F(hidden,evidence[evidence_dict[path]:evidence_dict[path]+1],attention_mask_passage[evidence_dict[path]:evidence_dict[path]+1])[0]
                        hidden = self.PathEncoder_F(hidden_,hidden)
            ...
            for k,nodepaths in tree.node2path_B.items():
                for nodepath in nodepaths:
                    ...
                    for path in nodepath[0]:
                        hidden = self.Decoder_B(hidden,evidence[evidence_dict[path]:evidence_dict[path]+1],attention_mask_passage[evidence_dict[path]:evidence_dict[path]+1])[0]
                    preds.extend(self.Lr_output_B(self.Decoder(evidence[batch_item*i:batch_item*(i+1)],hidden,attention_mask_passage[batch_item*i:batch_item*(i+1)])[0][:,0,:]))
                    label = torch.zeros(batch_item)
                    for path in nodepath[1]:
                        if path != 'Q':
                            label[evidence_dict[path]-batch_item*i] = 1
    ...
    def generate_recursion(self,hidden,evidence,attention_mask_passage,node,output,evidence_dict_EN,i,batch_item,ns_list):
        hidden_ = self.Decoder_F(hidden, evidence[node.item():node.item() + 1],
        ...
        for n in next_nodes.nonzero()[:, 0]:
            output.append([evidence_dict_EN[node.item()+i*batch_item],evidence_dict_EN[n.item()+i*batch_item]])
    ...
    def generate(
        ...
        for i in range(len(question)):
            ...
            for node in nearroot_nodes_i.nonzero()[:,0]:
                ...
                output = self.generate_recursion(H0_F,evidence,attention_mask_passage,node,output,evidence_dict_EN,i,batch_item,[node.item()])
